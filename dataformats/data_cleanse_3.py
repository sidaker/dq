...
# Add new field called Date Acquired which is a pure date.
#df3['PartDateAcquired'] = df3.loc[:, 'DateAcquired'].apply(lambda x: datevalidator2(x))
# print(time.strftime("%Y-%m-%d %H:%M:%S", ts))
# datetime.datetime.strptime(date_time_str, '%Y-%m-%d %H:%M:%S.%f').date()
# datetime.datetime.strptime("2020-04-30 05:01:14", '%Y-%m-%d %H:%M:%S').date()
#df3['PartDateAcquired'] = df3.loc[:, 'DateAcquired'].apply(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d %H:%M:%S').date())
df3['PartDateAcquired'] = df3.loc[:, 'DateAcquired'].apply(lambda x: datetime.datetime.strptime(x, '%d/%m/%Y %H:%M:%S').date())

print(df3['PartDateAcquired'].tail())
print(df3['DateofBirth'].tail())
print(df3.tail())
print(df3.dtypes)
#print(df3.info)
