...
sorted_by_name = sorted(people, key= lambda x:x['name'].lower())
...
name_declarations = list(map(lambda x: f'{x["name"]} is {x["age"]} years old', sorted_by_name))
...
under_seventy = sorted(list(filter(lambda x: x['age']<70, sorted_by_name)),key= lambda x: x['age'])
