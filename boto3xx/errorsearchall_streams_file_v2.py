...
python errorsearchall_streams_file_v2.py "/aws/lambda/api-record-level-score-test-lambda-athena"

python errorsearchall_streams_file_v2.py \
"/aws/lambda/internal-reporting-notprod-rds-lambda" \
--stream_prefix="2020/08/13/"
    ...
    with open(logfile, 'w', newline='') as file:
        ...
        for lstream in chunks:
            ...
            for event in get_error_log_events(env,log_group,lstream,limit):
                ...
                try:
                    if stats.startswith('{') :
                        # string with dictionary content.
                        #rstats1 = ast.literal_eval(stats)
                        rstats1 = re.search(r'\b(Status)\b', stats)
                        index= rstats1.start()
                        #print(stats[index:])
                        #rstats1 = ast.eval(stats)
                        rstats = stats[index:]
                        #rstats1 = ast.literal_eval(rstats)
                        #print(rstats1['StateChangeReason'])
                        #print("$$$$")
                        #print(rstats1)
                    else:
                        rstats = ''
